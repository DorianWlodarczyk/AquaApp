...
import AdminGroundApi from "./admin-ground-api.service";
...
const AdminGroundPage = () => {
  ...
  const renderTable = () => {
    const data = accessories?.grounds;
  ...
  const deleteRow = (id: string) => {
    ...
    AdminGroundApi.deleteGround(id);
    const newAccessories = accessories?.grounds?.filter(
      (item) => item.id !== activeID
    );

    if (accessories) {
      accessories.grounds = newAccessories || [];
  ...
  const editRow = (id: string) => {
    ...
    AdminGroundApi.editGround(id, newName);

    const newAccessories = { ...accessories };

    if (newAccessories.grounds) {
      for (let item of newAccessories.grounds) {
        if (item.id === activeID) {
          item.name = newName;
  ...
  const addRow = () => {
    AdminGroundApi.createGround(newName);
    const newAccessories = { ...accessories };

    newAccessories.grounds.push({
      id: `${new Date().getTime()}`,
  ...
  return (
    <Loader status={status}>
      <div className="p-8">
        <WidgetBox color={Color.ORANGE} title="Podłoże">
...
export default AdminGroundPage;

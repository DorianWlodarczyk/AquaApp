...
import AdminPumpApi from "./admin-pump-api.service";
...
const AdminPumpPage = () => {
  ...
  const deleteRow = (id: string) => {
    AdminPumpApi.deletePump(id);

    const newAccessories = accessories?.pumps?.filter(
      (item) => item.id !== activeID
    );

    if (accessories) {
      accessories.pumps = newAccessories || [];
      setAccessories({ ...accessories });
    }
  };

  const editRow = (id: string) => {
    AdminPumpApi.editPump(id, newName, newCapacity);

    const newAccessories = { ...accessories };

    if (newAccessories.pumps) {
      for (let item of newAccessories.pumps) {
        if (item.id === activeID) {
          item.name = newName;
  ...
  const addRow = () => {
    AdminPumpApi.addNewPump(newName, newCapacity);
    const newAccessories = { ...accessories };

    newAccessories.pumps.push({
      id: `${new Date().getTime()}`,
  ...
  return (
    <Loader status={status}>
      <div className="p-8">
        <WidgetBox color={Color.BLUE} title="Pompy">

...
import AdminHeaterApi from "../heater/admin-heater-api.service";
import AdminLampApi from "./admin-lamp-api.service";
...
const AdminLampPage = () => {
  ...
  const deleteRow = (id: string) => {
    ...
    AdminLampApi.deleteLamp(id);
    const newAccessories = accessories?.lamps?.filter(
      (item) => item.id !== activeID
    );

    if (accessories) {
      accessories.lamps = newAccessories || [];
  ...
  const editRow = (id: string) => {
    ...
    AdminLampApi.editLamp(id, newName);

    const newAccessories = { ...accessories };

    if (newAccessories.lamps) {
      for (let item of newAccessories.lamps) {
        if (item.id === activeID) {
          item.name = newName;
  ...
  const addRow = () => {
    AdminLampApi.createLamp(newName);
    const newAccessories = { ...accessories };

    newAccessories.lamps.push({
      id: `${new Date().getTime()}`,
  ...
  return (
    <Loader status={status}>
      <div className="p-8">
        <WidgetBox color={Color.BLUE} title="Lampy">

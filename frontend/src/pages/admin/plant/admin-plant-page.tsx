...
import AdminPlantApi from "./admin-plant-api.service";
...
const AdminPlantPage = () => {
  ...
  const deleteRow = (id: string) => {
    ...
    AdminPlantApi.deletePlant(id);
    const newAccessories = accessories?.plants?.filter(
      (item) => item.id !== activeID
    );

    if (accessories) {
      accessories.plants = newAccessories || [];
  ...
  const editRow = (id: string) => {
    ...
    AdminPlantApi.editPlant(id, newName);

    const newAccessories = { ...accessories };

    if (newAccessories.plants) {
      for (let item of newAccessories.plants) {
        if (item.id === activeID) {
          item.name = newName;
  ...
  const addRow = () => {
    AdminPlantApi.createPlant(newName);
    const newAccessories = { ...accessories };

    newAccessories.plants.push({
      id: `${new Date().getTime()}`,
  ...
  return (
    <Loader status={status}>
      <div className="p-8">
        <WidgetBox color={Color.ORANGE} title="Rośliny">

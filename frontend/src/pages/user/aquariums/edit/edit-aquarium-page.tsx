...
import NewAquariumApi from "./edit-aquarium-api.service";
import AccessoriesData from "../../../../utils/models/accessories/accessories-data";
import Step4 from "./steps/Step4";
import { useNavigate, useParams } from "react-router-dom";
import AquaApi from "../aqua/aqua-api.service";
...
const EditAquariumPage = () => {
  ...
  const saveAquarium = async () => {
    const z = await NewAquariumApi.editAquarium(id!, inputs);
    if (z) navigate(`/aqua/${z.aquariumID}`);
  };
      ...
      setInputs([
        {
          ...
          value: data?.aquaName,
          error: false,
          pattern: maxNameLength,
        },
        { name: "imgID", value: data.aquaImg, error: false },
        {
          name: "width",
          value: data.width.toString(),
          error: false,
          pattern: aquariumDimension,
        },
        {
          name: "height",
        ...
        {
          name: "heater",
          value: accessories.heaters.find(
            (item) => item.name === data.heaterName
          )?.id!,
          error: false,
        ...
        {
          name: "pump",
          value: accessories.pumps.find((item) => item.name === data.pumpName)
            ?.id!,
          error: false,
          pattern: anyCharacter,
        },
        {
          name: "lamp",
          value: accessories.lamps.find((item) => item.name === data.lampName)
            ?.id!,
          error: false,
          pattern: anyCharacter,
        },
        {
          name: "asset",
          value: accessories.assets.find((item) => item.name === data.assetName)
            ?.id!,
          error: false,
          pattern: anyCharacter,
        },
        {
          name: "plant",
          value: accessories.plants.find((item) => item.name === data.plantName)
            ?.id!,
          error: false,
          pattern: anyCharacter,
        },
        {
          name: "ground",
          value: accessories.grounds.find(
            (item) => item.name === data.groundName
          )?.id!,
          error: false,
          pattern: anyCharacter,
        },
